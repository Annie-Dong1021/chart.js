...
var _colorName_1_1_3_colorName = {
...
var reverseKeywords = {};
for (var key in _colorName_1_1_3_colorName) {
	if (_colorName_1_1_3_colorName.hasOwnProperty(key)) {
		reverseKeywords[_colorName_1_1_3_colorName[key]] = key;
	}
}
...
convert.rgb.keyword = function (rgb) {
	...
	for (var keyword in _colorName_1_1_3_colorName) {
		if (_colorName_1_1_3_colorName.hasOwnProperty(keyword)) {
			var value = _colorName_1_1_3_colorName[keyword];
...
convert.keyword.rgb = function (keyword) {
	return _colorName_1_1_3_colorName[keyword];
};
...
var _colorConvert_1_9_3_colorConvert = convert;
...
Color.prototype.setValues = function (space, vals) {
	...
	for (var sname in spaces) {
		if (sname !== space) {
			values[sname] = _colorConvert_1_9_3_colorConvert[space][sname](values[space]);
		}
	}
...
var _chartjsColor_2_4_1_chartjsColor = Color;
...
function interpolate(start, view, model, ease) {
	...
	for (i = 0, ilen = keys.length; i < ilen; ++i) {
		...
		if (type === typeof origin) {
			if (type === 'string') {
				c0 = _chartjsColor_2_4_1_chartjsColor(origin);
				if (c0.valid) {
					c1 = _chartjsColor_2_4_1_chartjsColor(target);
					if (c1.valid) {
 ...
 */
function initCanvas(canvas, config, context) {
...
var platform_dom$2 = {
	...
	acquireContext: function(item, config) {
		...
		if (context && context.canvas === item) {
			...
			initCanvas(item, config, context);
			return context;
		}
	...
	addEventListener: function(chart, type, listener) {
		...
		if(type == 'mouseout' && typeof(api) == 'object' && api.platform == 'mp'){
		...
		var proxy = proxies[chart.id + '_' + type] = function(event) {
			if(typeof(api) == 'object' && api.platform == 'mp'){
				canvas.$$getBoundingClientRect().then(res => {
					chart.canvas.boundingClientRect = res;
					listener(fromNativeEvent(event, chart));
					chart.canvas.boundingClientRect = undefined;
				});
			}else {
				chart.canvas.boundingClientRect = canvas.getBoundingClientRect();
				listener(fromNativeEvent(event, chart));
				chart.canvas.boundingClientRect = undefined;
			}
		};
...
helpers$1.extend(Chart.prototype, /** @lends Chart */ {
	...
	construct: function(item, config) {
		...
		function init(item, config){
			var me = this;
			me.k_canvas = item.k_canvas;
...
var core_helpers = function() {
	...
	helpers$1.getRelativePosition = function(evt, chart) {
		...
		var boundingRect = chart.canvas.boundingClientRect || canvas.getBoundingClientRect();
	...
	helpers$1.color = !_chartjsColor_2_4_1_chartjsColor ?
		function(value) {
		...
		function(value) {
			...
			return _chartjsColor_2_4_1_chartjsColor(value);
		};
